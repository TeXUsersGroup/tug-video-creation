#!/usr/bin/env python3
"""
Fetch the TUG 2026 abstracts from tug.org and turn each into a plain-text
YouTube description.

For every talk in talks.tsv it downloads
    https://tug.org/tug2026/abstracts/<name>.txt
...
BASE = "https://tug.org/tug2026/abstracts"
PAPERS_BASE = "https://tug.org/TUGboat/tb47-2"   # TUGboat 47:2 (TUG 2026 papers)
...
WORDS = {
    ...
    # 2026 abstracts: acronyms/product names that just render as their letters
    "ASCII": "ASCII", "GL": "GL", "GPU": "GPU", "GUI": "GUI", "JSON": "JSON",
    "STEM": "STEM", "WEB": "WEB", "macOS": "macOS", "LaTeXML": "LaTeXML",
    # 2026 abstracts: macros whose expansion is NOT just the letters
    "LaTeXe": "LaTeX2e", "PDFUA": "PDF/UA", "tbUTF": "UTF", "longrightarrow": "→",
}
# \name{arg} macros -> keep arg 1 (the text); titleref = a referenced title
...
def clean(body):
    ...
    # \cs{section} names a control sequence: render it as "\section". Use a
    # backslash sentinel (\x02) so the literal backslash survives the macro
    # stripping below; it is restored to "\" at the end.
    BS = '\x02'
    t = re.sub(r'\\cs\s*\{([^{}]*)\}', lambda m: BS + m.group(1), t)

    # \char`_ (or \char`\_) is TeX for "the literal character _" (used inside
    # \texttt code spans, e.g. pre\char`_linebreak -> pre_linebreak).
    t = re.sub(r"\\char`\\?(.)", r"\1", t)
    ...
    t = t.replace('---', '—').replace('--', '–')
    t = t.replace('``', '“').replace("''", '”')    # TeX double quotes -> curly
    t = t.replace('$', ' ')                        # drop leftover math delimiters
    t = t.replace('{', '').replace('}', '')
    t = t.replace(NB, ' ').replace(NL, '\n')      # restore protected spacing
    t = t.replace(BS, '\\')                       # restore \cs backslashes
...
def write_desc(token, desc, paper_pdf):
    ...
    if paper_pdf and paper_pdf != '-':
        parts.append(f"Paper (TUGboat 47:2): {PAPERS_BASE}/{paper_pdf}")
    if desc:
        parts.append(desc)
    text = "\n\n".join(parts)
